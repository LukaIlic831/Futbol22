    ...
    <>
      {givedUp ? (
        <p className="finished__text">
          You lost. Come back tomorrow to try again.
        </p>
      ) : counter == 11 || localStorage.futbol22GameFinished ? (
        <p className="finished__text">Come back tomorrow to try again.</p>
      ) : (
        <div className="blocks__wrapper">
          {countries?.slice(counter, counter + 1).map((item) => (
            <div className="country">
              <img src={flags[item.extra.fifa]} alt="" />
              <p>{item.name}</p>
            </div>
          ))}
          ...
          <div className="search__input--text">
            {guessButtonClicked &&
              !playerInFormation &&
              (player?.display_name ? (
                <p className="search__input--text-para">
                  Did you mean {player.display_name}
                </p>
              ) : (
                <p className="search__input--text-para">
            ...
            {playerInFormation && (
              <p className="search__input--text-para">
                {player?.display_name} is already in formation
              </p>
            )}
          </div>
        </div>
...
export default Futbol22GuessBlocks;
